...
@dataclass
class DownstreamLoop(MainCliArgs):
    ...
    def execute(
        ...
        root_dir = self.output or self.lens / "downstream"
        root_dir.mkdir(exist_ok=True, parents=True)

        tasks = get_task_dict(self.task)
        if len(tasks) > 1:
        ...
        for task_name, task in tasks.items():
            ...
            if isinstance(dataset, Dataset):
                ...
                if self.limit and len(dataset) > self.limit:
                    print(f"Limiting dataset to {self.limit} examples")
            ...
            for doc in pbar:
                ...
                if self.injection:
                    assert self.num_shots > 0, "Cannot inject zero-shot prompt"
                ...
                pbar.set_postfix(
                    {k: agg_fns[k](v) for k, v in layer_metrics[-1].items()}
                )
            ...
            for i, metric_dict in enumerate(layer_metrics):
                for metric_name, items in metric_dict.items():
                    ...
                    if local_rank == 0:
                        agg_metrics[i][metric_name] = task.aggregation()[metric_name](
                            items
                        )
